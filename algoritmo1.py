...
# Guardar textos clasificados como 'Education'
education_texts = []
...
with open('corpus.jsonl', 'r', encoding='utf-8') as f:
    for line in f:
        data = json.loads(line.strip())
        text = data.get('text', '')
        if not text or 'domain' not in data or data['domain'] not in {'ingles', 'outfox'}:
            continue
        tokens = preprocess_text(text)
        lemmatized_tokens = lemmatize_tokens(tokens)
        category = determine_category(lemmatized_tokens)
...
with open('education_texts.json', 'w', encoding='utf-8') as out:
    json.dump(education_texts, out, ensure_ascii=False, indent=2)
print(f"Se han guardado {len(education_texts)} textos educativos en 'education_texts.json'")
